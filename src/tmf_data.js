// Auto-generated by tmf-spec-parser v0.2.6
// Do not edit manually — run: tmf-spec-parser generate

export default {
  "generated_at": "2026-04-26T15:25:07Z",
  "parser_version": "0.2.6",
  "apis": [
    {
      "id": "TMF629",
      "name": "Customer Management",
      "domain": "customer",
      "short": "Customer Mgmt",
      "repo": "TMF629_CustomerManagement"
    },
    {
      "id": "TMF620",
      "name": "Product Catalog",
      "domain": "product",
      "short": "Product Catalog",
      "repo": "TMF620_ProductCatalog"
    },
    {
      "id": "TMF622",
      "name": "Product Ordering",
      "domain": "product",
      "short": "Product Ordering",
      "repo": "TMF622_ProductOrder"
    },
    {
      "id": "TMF637",
      "name": "Product Inventory",
      "domain": "product",
      "short": "Product Inventory",
      "repo": "TMF637_ProductInventory"
    },
    {
      "id": "TMF633",
      "name": "Service Catalog",
      "domain": "service",
      "short": "Service Catalog",
      "repo": "TMF633_ServiceCatalog"
    },
    {
      "id": "TMF638",
      "name": "Service Inventory",
      "domain": "service",
      "short": "Service Inventory",
      "repo": "TMF638_ServiceInventory"
    },
    {
      "id": "TMF641",
      "name": "Service Ordering",
      "domain": "service",
      "short": "Service Ordering",
      "repo": "TMF641_ServiceOrder"
    },
    {
      "id": "TMF645",
      "name": "Service Qualification",
      "domain": "service",
      "short": "Service Qual",
      "repo": "TMF645_ServiceQualification"
    },
    {
      "id": "TMF653",
      "name": "Service Test",
      "domain": "service",
      "short": "Service Test",
      "repo": "TMF653_ServiceTestManagement"
    },
    {
      "id": "TMF634",
      "name": "Resource Catalog",
      "domain": "resource",
      "short": "Resource Catalog",
      "repo": "TMF634_ResourceCatalog"
    },
    {
      "id": "TMF639",
      "name": "Resource Inventory",
      "domain": "resource",
      "short": "Resource Inventory",
      "repo": "TMF639_ResourceInventory"
    },
    {
      "id": "TMF652",
      "name": "Resource Ordering",
      "domain": "resource",
      "short": "Resource Ordering",
      "repo": "TMF652_ResourceOrderManagement"
    },
    {
      "id": "TMF621",
      "name": "Trouble Ticket",
      "domain": "engagement",
      "short": "Trouble Ticket",
      "repo": "TMF621_TroubleTicket"
    },
    {
      "id": "TMF656",
      "name": "Service Problem",
      "domain": "engagement",
      "short": "Service Problem",
      "repo": "TMF656_ServiceProblemManagement"
    },
    {
      "id": "TMF632",
      "name": "Party Management",
      "domain": "common",
      "short": "Party Mgmt",
      "repo": "TMF632_PartyManagement"
    },
    {
      "id": "TMF688",
      "name": "Event Management",
      "domain": "common",
      "short": "Event Mgmt",
      "repo": "TMF688-Event"
    }
  ],
  "links": [
    {
      "source": "TMF620",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF620",
      "target": "TMF633",
      "label": "references ServiceSpec"
    },
    {
      "source": "TMF620",
      "target": "TMF634",
      "label": "references ResourceSpec"
    },
    {
      "source": "TMF621",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF622",
      "target": "TMF620",
      "label": "references ProductOffering"
    },
    {
      "source": "TMF622",
      "target": "TMF620",
      "label": "references ProductSpec"
    },
    {
      "source": "TMF622",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF622",
      "target": "TMF638",
      "label": "references Service"
    },
    {
      "source": "TMF622",
      "target": "TMF639",
      "label": "references Resource"
    },
    {
      "source": "TMF629",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF633",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF633",
      "target": "TMF634",
      "label": "references ResourceSpec"
    },
    {
      "source": "TMF634",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF637",
      "target": "TMF620",
      "label": "references ProductOffering"
    },
    {
      "source": "TMF637",
      "target": "TMF620",
      "label": "references ProductSpec"
    },
    {
      "source": "TMF637",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF637",
      "target": "TMF638",
      "label": "references Service"
    },
    {
      "source": "TMF637",
      "target": "TMF639",
      "label": "references Resource"
    },
    {
      "source": "TMF638",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF638",
      "target": "TMF633",
      "label": "references ServiceSpec"
    },
    {
      "source": "TMF638",
      "target": "TMF639",
      "label": "references Resource"
    },
    {
      "source": "TMF639",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF639",
      "target": "TMF634",
      "label": "references ResourceSpec"
    },
    {
      "source": "TMF641",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF641",
      "target": "TMF633",
      "label": "references ServiceSpec"
    },
    {
      "source": "TMF641",
      "target": "TMF639",
      "label": "references Resource"
    },
    {
      "source": "TMF645",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF645",
      "target": "TMF633",
      "label": "references ServiceSpec"
    },
    {
      "source": "TMF645",
      "target": "TMF639",
      "label": "references Resource"
    },
    {
      "source": "TMF652",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF652",
      "target": "TMF634",
      "label": "references ResourceSpec"
    },
    {
      "source": "TMF653",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF653",
      "target": "TMF633",
      "label": "references ServiceSpec"
    },
    {
      "source": "TMF653",
      "target": "TMF638",
      "label": "references Service"
    },
    {
      "source": "TMF656",
      "target": "TMF621",
      "label": "references TroubleTicket"
    },
    {
      "source": "TMF656",
      "target": "TMF632",
      "label": "has RelatedParty"
    },
    {
      "source": "TMF656",
      "target": "TMF638",
      "label": "references Service"
    },
    {
      "source": "TMF656",
      "target": "TMF639",
      "label": "references Resource"
    },
    {
      "source": "TMF688",
      "target": "TMF632",
      "label": "has RelatedParty"
    }
  ],
  "patterns": [
    {
      "id": "o2a",
      "name": "Order-to-Activate",
      "color": "#ff9f0a",
      "nodes": [
        "TMF629",
        "TMF622",
        "TMF641",
        "TMF638",
        "TMF639"
      ],
      "desc": "Customer → Product Order → Service Order → Service → Resource"
    },
    {
      "id": "c2i",
      "name": "Catalog-to-Inventory",
      "color": "#5e9bff",
      "nodes": [
        "TMF620",
        "TMF633",
        "TMF634"
      ],
      "desc": "Sync catalog specifications with provisioned inventory instances"
    },
    {
      "id": "t2r",
      "name": "Trouble-to-Resolve",
      "color": "#ff6b6b",
      "nodes": [
        "TMF621",
        "TMF656",
        "TMF638"
      ],
      "desc": "Incident detection → problem management → service resolution"
    }
  ],
  "details": {
    "TMF629": {
      "specRef": "TMF629 v4.0",
      "description": "environment TMF Customer Management specification",
      "entities": [
        {
          "name": "Customer",
          "mandatory": [
            "engagedParty"
          ],
          "optional": [
            "id",
            "href",
            "name",
            "status",
            "statusReason",
            "account",
            "agreement",
            "characteristic",
            "contactMedium",
            "creditProfile",
            "paymentMethod",
            "relatedParty"
          ]
        },
        {
          "name": "CustomerCreateEventPayload",
          "mandatory": [],
          "optional": [
            "customer"
          ]
        },
        {
          "name": "CustomerAttributeValueChangeEventPayload",
          "mandatory": [],
          "optional": [
            "customer"
          ]
        },
        {
          "name": "CustomerStateChangeEventPayload",
          "mandatory": [],
          "optional": [
            "customer"
          ]
        }
      ],
      "lifecycle": [
        "pending",
        "approved",
        "inactive",
        "terminated"
      ],
      "terminal": [
        "terminated"
      ],
      "transitions": [
        {
          "from": "pending",
          "to": "approved"
        },
        {
          "from": "pending",
          "to": "inactive"
        },
        {
          "from": "approved",
          "to": "inactive"
        },
        {
          "from": "inactive",
          "to": "terminated"
        }
      ]
    },
    "TMF620": {
      "specRef": "TMF620 v4.1.0",
      "description": "Product Catalog Management Release : 20.",
      "entities": [
        {
          "name": "Catalog",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "catalogType",
            "description",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "version",
            "category",
            "relatedParty",
            "validFor",
            "@baseType"
          ]
        },
        {
          "name": "ProductOffering",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "isBundle",
            "isSellable",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "statusReason",
            "version",
            "agreement",
            "attachment"
          ]
        },
        {
          "name": "ProductOfferingPrice",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "isBundle",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "percentage",
            "priceType",
            "recurringChargePeriodLength",
            "recurringChargePeriodType",
            "version"
          ]
        },
        {
          "name": "ProductOfferingPriceRefOrValue",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "priceType",
            "recurringChargePeriod",
            "recurringChargePeriodLength",
            "version",
            "constraint",
            "price"
          ]
        }
      ],
      "lifecycle": [
        "inStudy",
        "inDesign",
        "inTest",
        "active",
        "launched",
        "retired",
        "obsolete"
      ],
      "terminal": [
        "retired",
        "obsolete"
      ],
      "transitions": [
        {
          "from": "inStudy",
          "to": "inDesign"
        },
        {
          "from": "inDesign",
          "to": "inTest"
        },
        {
          "from": "inTest",
          "to": "active"
        },
        {
          "from": "inTest",
          "to": "inStudy"
        },
        {
          "from": "active",
          "to": "launched"
        },
        {
          "from": "launched",
          "to": "active"
        },
        {
          "from": "launched",
          "to": "retired"
        },
        {
          "from": "launched",
          "to": "obsolete"
        },
        {
          "from": "retired",
          "to": "obsolete"
        }
      ]
    },
    "TMF622": {
      "specRef": "TMF622 v4.0.0",
      "description": "Product Ordering Management Release : 19.",
      "entities": [
        {
          "name": "ProductOrderItem",
          "mandatory": [
            "id",
            "action"
          ],
          "optional": [
            "quantity",
            "appointment",
            "billingAccount",
            "itemPrice",
            "itemTerm",
            "itemTotalPrice",
            "payment",
            "product",
            "productOffering",
            "productOfferingQualificationItem",
            "productOrderItem",
            "productOrderItemRelationship"
          ]
        },
        {
          "name": "ProductOrder",
          "mandatory": [
            "productOrderItem"
          ],
          "optional": [
            "id",
            "href",
            "cancellationDate",
            "cancellationReason",
            "category",
            "completionDate",
            "description",
            "expectedCompletionDate",
            "externalId",
            "notificationContact",
            "orderDate",
            "priority"
          ]
        },
        {
          "name": "CancelProductOrder",
          "mandatory": [
            "productOrder"
          ],
          "optional": [
            "id",
            "href",
            "cancellationReason",
            "effectiveCancellationDate",
            "requestedCancellationDate",
            "state",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "RelatedProductOrderItem",
          "mandatory": [
            "orderItemId",
            "productOrderId"
          ],
          "optional": [
            "orderItemAction",
            "productOrderHref",
            "role",
            "@baseType",
            "@schemaLocation",
            "@type",
            "@referredType"
          ]
        }
      ],
      "lifecycle": [
        "acknowledged",
        "rejected",
        "pending",
        "held",
        "inProgress",
        "cancelled",
        "completed",
        "failed",
        "assessingCancellation",
        "pendingCancellation"
      ],
      "terminal": [
        "completed",
        "failed",
        "cancelled"
      ],
      "transitions": [
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "acknowledged",
          "to": "cancelled"
        },
        {
          "from": "inProgress",
          "to": "pending"
        },
        {
          "from": "inProgress",
          "to": "held"
        },
        {
          "from": "inProgress",
          "to": "completed"
        },
        {
          "from": "inProgress",
          "to": "failed"
        },
        {
          "from": "inProgress",
          "to": "cancelled"
        },
        {
          "from": "pending",
          "to": "inProgress"
        },
        {
          "from": "held",
          "to": "inProgress"
        }
      ]
    },
    "TMF637": {
      "specRef": "TMF637 v4.0.0",
      "description": "Product Inventory Management Release : 19.",
      "entities": [
        {
          "name": "Product",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "isBundle",
            "isCustomerVisible",
            "name",
            "orderDate",
            "productSerialNumber",
            "startDate",
            "terminationDate",
            "agreement",
            "billingAccount"
          ]
        },
        {
          "name": "ProductPrice",
          "mandatory": [
            "priceType",
            "price"
          ],
          "optional": [
            "description",
            "name",
            "recurringChargePeriod",
            "unitOfMeasure",
            "billingAccount",
            "productOfferingPrice",
            "productPriceAlteration",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "ProductRefOrValue",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "isBundle",
            "isCustomerVisible",
            "name",
            "orderDate",
            "productSerialNumber",
            "startDate",
            "terminationDate",
            "agreement",
            "billingAccount"
          ]
        },
        {
          "name": "RelatedProductOrderItem",
          "mandatory": [
            "orderItemId",
            "productOrderId"
          ],
          "optional": [
            "orderItemAction",
            "productOrderHref",
            "role",
            "@baseType",
            "@schemaLocation",
            "@type",
            "@referredType"
          ]
        }
      ],
      "lifecycle": [
        "created",
        "pendingActive",
        "cancelled",
        "active",
        "pendingTerminate",
        "terminated",
        "suspended",
        "aborted "
      ],
      "terminal": [
        "terminated"
      ],
      "transitions": [
        {
          "from": "created",
          "to": "pending"
        },
        {
          "from": "pending",
          "to": "active"
        },
        {
          "from": "pending",
          "to": "inactive"
        },
        {
          "from": "active",
          "to": "inactive"
        },
        {
          "from": "inactive",
          "to": "active"
        },
        {
          "from": "active",
          "to": "terminated"
        },
        {
          "from": "inactive",
          "to": "terminated"
        }
      ]
    },
    "TMF633": {
      "specRef": "TMF633 v4.0.0",
      "description": "Service Catalog Management Version 4.",
      "entities": [
        {
          "name": "ServiceCatalog",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "version",
            "category",
            "relatedParty",
            "validFor",
            "@baseType",
            "@schemaLocation"
          ]
        },
        {
          "name": "ServiceCatalogCreateEventPayload",
          "mandatory": [],
          "optional": [
            "serviceCatalog"
          ]
        },
        {
          "name": "ServiceCatalogChangeEventPayload",
          "mandatory": [],
          "optional": [
            "serviceCatalog"
          ]
        },
        {
          "name": "ServiceCatalogBatchEventPayload",
          "mandatory": [],
          "optional": [
            "serviceCatalog"
          ]
        }
      ],
      "lifecycle": [
        "inStudy",
        "inDesign",
        "inTest",
        "active",
        "launched",
        "retired"
      ],
      "terminal": [
        "retired"
      ],
      "transitions": [
        {
          "from": "inStudy",
          "to": "inDesign"
        },
        {
          "from": "inDesign",
          "to": "inTest"
        },
        {
          "from": "inTest",
          "to": "active"
        },
        {
          "from": "inTest",
          "to": "inStudy"
        },
        {
          "from": "active",
          "to": "launched"
        },
        {
          "from": "launched",
          "to": "retired"
        }
      ]
    },
    "TMF638": {
      "specRef": "TMF638 v4.0.0",
      "description": "Service Inventory Management Version 4.",
      "entities": [
        {
          "name": "ServiceRefOrValue",
          "mandatory": [
            "id"
          ],
          "optional": [
            "href",
            "category",
            "description",
            "endDate",
            "hasStarted",
            "isBundle",
            "isServiceEnabled",
            "isStateful",
            "name",
            "serviceDate",
            "serviceType",
            "startDate"
          ]
        },
        {
          "name": "Service",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "category",
            "description",
            "endDate",
            "hasStarted",
            "isBundle",
            "isServiceEnabled",
            "isStateful",
            "name",
            "serviceDate",
            "serviceType"
          ]
        },
        {
          "name": "RelatedServiceOrderItem",
          "mandatory": [
            "itemId",
            "serviceOrderId"
          ],
          "optional": [
            "role",
            "serviceOrderHref",
            "itemAction",
            "@referredType"
          ]
        },
        {
          "name": "ServiceRelationship",
          "mandatory": [
            "relationshipType",
            "service"
          ],
          "optional": [
            "serviceRelationshipCharacteristic"
          ]
        }
      ],
      "lifecycle": [
        "feasibilityChecked",
        "designed",
        "reserved",
        "inactive",
        "active",
        "terminated"
      ],
      "terminal": [
        "terminated"
      ],
      "transitions": [
        {
          "from": "feasibilityChecked",
          "to": "designed"
        },
        {
          "from": "designed",
          "to": "reserved"
        },
        {
          "from": "reserved",
          "to": "active"
        },
        {
          "from": "active",
          "to": "inactive"
        },
        {
          "from": "inactive",
          "to": "active"
        },
        {
          "from": "active",
          "to": "terminated"
        },
        {
          "from": "inactive",
          "to": "terminated"
        }
      ]
    },
    "TMF641": {
      "specRef": "TMF641 v4.1.0",
      "description": "Service Ordering Management Version 4.",
      "entities": [
        {
          "name": "CancelServiceOrder",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "cancellationReason",
            "completionMessage",
            "effectiveCancellationDate",
            "requestedCancellationDate",
            "errorMessage",
            "serviceOrder",
            "state",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "ServiceOrder",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "cancellationDate",
            "cancellationReason",
            "category",
            "completionDate",
            "description",
            "expectedCompletionDate",
            "externalId",
            "notificationContact",
            "orderDate",
            "priority"
          ]
        },
        {
          "name": "ServiceOrderItem",
          "mandatory": [
            "id",
            "action",
            "service"
          ],
          "optional": [
            "quantity",
            "appointment",
            "errorMessage",
            "serviceOrderItem",
            "serviceOrderItemRelationship",
            "state",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "RelatedServiceOrderItem",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "itemId",
            "role",
            "serviceOrderHref",
            "serviceOrderId",
            "itemAction",
            "@baseType",
            "@schemaLocation",
            "@type",
            "@referredType"
          ]
        }
      ],
      "lifecycle": [
        "accepted",
        "terminatedWithError",
        "inProgress",
        "done"
      ],
      "terminal": [
        "completed",
        "failed",
        "cancelled"
      ],
      "transitions": [
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "acknowledged",
          "to": "cancelled"
        },
        {
          "from": "inProgress",
          "to": "pending"
        },
        {
          "from": "inProgress",
          "to": "held"
        },
        {
          "from": "inProgress",
          "to": "completed"
        },
        {
          "from": "inProgress",
          "to": "failed"
        },
        {
          "from": "inProgress",
          "to": "cancelled"
        },
        {
          "from": "pending",
          "to": "inProgress"
        },
        {
          "from": "held",
          "to": "inProgress"
        }
      ]
    },
    "TMF645": {
      "specRef": "TMF645 v4.0.0",
      "description": "Service Qualification Version 4.",
      "entities": [
        {
          "name": "CheckServiceQualification",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "checkServiceQualificationDate",
            "description",
            "effectiveQualificationDate",
            "estimatedResponseDate",
            "expectedQualificationDate",
            "expirationDate",
            "externalId",
            "instantSyncQualification",
            "provideAlternative",
            "provideUnavailabilityReason"
          ]
        },
        {
          "name": "CheckServiceQualificationItem",
          "mandatory": [],
          "optional": [
            "id",
            "expectedActivationDate",
            "expectedServiceAvailabilityDate",
            "expirationDate",
            "qualificationResult",
            "state",
            "alternateServiceProposal",
            "category",
            "eligibilityUnavailabilityReason",
            "qualificationItemRelationship",
            "qualificationRelationship",
            "service"
          ]
        },
        {
          "name": "QueryServiceQualification",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "effectiveQualificationDate",
            "estimatedResponseDate",
            "expectedQualificationDate",
            "expirationDate",
            "externalId",
            "instantSyncQualification",
            "queryServiceQualificationDate",
            "relatedParty",
            "searchCriteria"
          ]
        },
        {
          "name": "ServiceQualification",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "effectiveQualificationDate",
            "estimatedResponseDate",
            "expectedQualificationDate",
            "expirationDate",
            "externalId",
            "instantSyncQualification",
            "relatedParty",
            "state",
            "@baseType"
          ]
        }
      ],
      "lifecycle": [
        "accepted",
        "terminatedWithError",
        "inProgress",
        "done"
      ],
      "terminal": [
        "done.unableToProvide",
        "done.provideAlternative",
        "done.standard"
      ],
      "transitions": [
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "inProgress",
          "to": "done.standard"
        },
        {
          "from": "inProgress",
          "to": "done.provideAlternative"
        },
        {
          "from": "inProgress",
          "to": "done.unableToProvide"
        }
      ]
    },
    "TMF653": {
      "specRef": "TMF653 v4.2.0",
      "description": "Service Test Management Release : 20.",
      "entities": [
        {
          "name": "ServiceTest",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "endDateTime",
            "mode",
            "name",
            "startDateTime",
            "state",
            "characteristic",
            "relatedParty",
            "relatedService",
            "testMeasure"
          ]
        },
        {
          "name": "ServiceTestSpecification",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "isBundle",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "version",
            "attachment",
            "constraint",
            "entitySpecRelationship",
            "relatedParty"
          ]
        },
        {
          "name": "ServiceTestSpecRelationship",
          "mandatory": [
            "relationshipType"
          ],
          "optional": [
            "id",
            "href",
            "name",
            "role",
            "validFor",
            "@baseType",
            "@schemaLocation",
            "@type",
            "@referredType"
          ]
        },
        {
          "name": "ServiceTestCreateEventPayload",
          "mandatory": [],
          "optional": [
            "serviceTest"
          ]
        }
      ],
      "lifecycle": [
        "acknowledged",
        "inProgress",
        "completed",
        "failed",
        "cancelled"
      ],
      "terminal": [
        "completed",
        "failed",
        "cancelled"
      ],
      "transitions": [
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "acknowledged",
          "to": "cancelled"
        },
        {
          "from": "inProgress",
          "to": "completed"
        },
        {
          "from": "inProgress",
          "to": "failed"
        },
        {
          "from": "inProgress",
          "to": "cancelled"
        }
      ]
    },
    "TMF634": {
      "specRef": "TMF634 v4.1.0",
      "description": "Resource Catalog Management December 2019 Resource Catalog API is one of Catalog Management API Family.",
      "entities": [
        {
          "name": "ResourceCatalog",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "description",
            "lastUpdate",
            "lifecycleStatus",
            "name",
            "version",
            "category",
            "relatedParty",
            "validFor",
            "@baseType",
            "@schemaLocation"
          ]
        },
        {
          "name": "ResourceCatalogCreateEventPayload",
          "mandatory": [],
          "optional": [
            "resourceCatalog"
          ]
        },
        {
          "name": "ResourceCatalogChangeEventPayload",
          "mandatory": [],
          "optional": [
            "resourceCatalog"
          ]
        },
        {
          "name": "ResourceCatalogDeleteEventPayload",
          "mandatory": [],
          "optional": [
            "resourceCatalog"
          ]
        }
      ],
      "lifecycle": [
        "inStudy",
        "inDesign",
        "inTest",
        "active",
        "launched",
        "retired"
      ],
      "terminal": [
        "retired"
      ],
      "transitions": [
        {
          "from": "inStudy",
          "to": "inDesign"
        },
        {
          "from": "inDesign",
          "to": "inTest"
        },
        {
          "from": "inTest",
          "to": "active"
        },
        {
          "from": "inTest",
          "to": "inStudy"
        },
        {
          "from": "active",
          "to": "launched"
        },
        {
          "from": "launched",
          "to": "retired"
        }
      ]
    },
    "TMF639": {
      "specRef": "TMF639 v4.0.0",
      "description": "Resource Inventory Release : 19.",
      "entities": [
        {
          "name": "LogicalResource",
          "mandatory": [
            "id",
            "href"
          ],
          "optional": [
            "category",
            "description",
            "endOperatingDate",
            "name",
            "resourceVersion",
            "startOperatingDate",
            "value",
            "activationFeature",
            "administrativeState",
            "attachment",
            "note",
            "operationalState"
          ]
        },
        {
          "name": "PhysicalResource",
          "mandatory": [
            "id",
            "href"
          ],
          "optional": [
            "category",
            "description",
            "endOperatingDate",
            "manufactureDate",
            "name",
            "powerState",
            "resourceVersion",
            "serialNumber",
            "startOperatingDate",
            "versionNumber",
            "activationFeature",
            "administrativeState"
          ]
        },
        {
          "name": "Resource",
          "mandatory": [
            "id",
            "href"
          ],
          "optional": [
            "category",
            "description",
            "endOperatingDate",
            "name",
            "resourceVersion",
            "startOperatingDate",
            "activationFeature",
            "administrativeState",
            "attachment",
            "note",
            "operationalState",
            "place"
          ]
        },
        {
          "name": "ResourceRefOrValue",
          "mandatory": [
            "id",
            "href"
          ],
          "optional": [
            "category",
            "description",
            "endOperatingDate",
            "name",
            "resourceVersion",
            "startOperatingDate",
            "activationFeature",
            "administrativeState",
            "attachment",
            "note",
            "operationalState",
            "place"
          ]
        }
      ],
      "lifecycle": [
        "standby",
        "alarm",
        "available",
        "reserved",
        "unknown",
        "suspended"
      ],
      "terminal": [
        "retired"
      ],
      "transitions": [
        {
          "from": "standby",
          "to": "available"
        },
        {
          "from": "available",
          "to": "reserved"
        },
        {
          "from": "reserved",
          "to": "available"
        },
        {
          "from": "available",
          "to": "suspended"
        },
        {
          "from": "suspended",
          "to": "available"
        },
        {
          "from": "available",
          "to": "alarm"
        },
        {
          "from": "alarm",
          "to": "available"
        },
        {
          "from": "available",
          "to": "retired"
        },
        {
          "from": "reserved",
          "to": "retired"
        },
        {
          "from": "suspended",
          "to": "retired"
        }
      ]
    },
    "TMF652": {
      "specRef": "TMF652 v4.0.0",
      "description": "environment TMF Resource Ordering Management specification",
      "entities": [
        {
          "name": "ResourceOrder",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "category",
            "completionDate",
            "description",
            "expectedCompletionDate",
            "externalId",
            "name",
            "orderDate",
            "orderType",
            "priority",
            "requestedCompletionDate"
          ]
        },
        {
          "name": "ResourceOrderItem",
          "mandatory": [],
          "optional": [
            "id",
            "action",
            "quantity",
            "state",
            "appointment",
            "orderItemRelationship",
            "resource",
            "resourceSpecification",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "CancelResourceOrder",
          "mandatory": [
            "resourceOrder"
          ],
          "optional": [
            "id",
            "href",
            "cancellationReason",
            "effectiveCancellationDate",
            "requestedCancellationDate",
            "state",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "ResourceOrderItemRelationship",
          "mandatory": [],
          "optional": [
            "relationshipType",
            "orderItem",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        }
      ],
      "lifecycle": [
        "acknowledged",
        "terminatedWithError",
        "inProgress",
        "done"
      ],
      "terminal": [
        "completed",
        "failed",
        "cancelled"
      ],
      "transitions": [
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "acknowledged",
          "to": "cancelled"
        },
        {
          "from": "inProgress",
          "to": "pending"
        },
        {
          "from": "inProgress",
          "to": "held"
        },
        {
          "from": "inProgress",
          "to": "completed"
        },
        {
          "from": "inProgress",
          "to": "failed"
        },
        {
          "from": "inProgress",
          "to": "cancelled"
        },
        {
          "from": "pending",
          "to": "inProgress"
        },
        {
          "from": "held",
          "to": "inProgress"
        }
      ]
    },
    "TMF621": {
      "specRef": "TMF621 v4.0.0",
      "description": "TMF - 621 Trouble Ticket Release : 19.",
      "entities": [
        {
          "name": "TroubleTicket",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "creationDate",
            "description",
            "expectedResolutionDate",
            "externalId",
            "lastUpdate",
            "name",
            "priority",
            "requestedResolutionDate",
            "resolutionDate",
            "severity"
          ]
        },
        {
          "name": "TroubleTicketRelationship",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "name",
            "relationshipType",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "TroubleTicketCreateEventPayload",
          "mandatory": [],
          "optional": [
            "troubleTicket"
          ]
        },
        {
          "name": "TroubleTicketAttributeValueChangeEventPayload",
          "mandatory": [],
          "optional": [
            "troubleTicket"
          ]
        }
      ],
      "lifecycle": [
        "acknowledged",
        "rejected",
        "pending",
        "held",
        "inProgress",
        "cancelled",
        "closed",
        "resolved"
      ],
      "terminal": [
        "closed",
        "cancelled"
      ],
      "transitions": [
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "acknowledged",
          "to": "cancelled"
        },
        {
          "from": "inProgress",
          "to": "pending"
        },
        {
          "from": "inProgress",
          "to": "held"
        },
        {
          "from": "inProgress",
          "to": "resolved"
        },
        {
          "from": "pending",
          "to": "inProgress"
        },
        {
          "from": "held",
          "to": "inProgress"
        },
        {
          "from": "resolved",
          "to": "closed"
        },
        {
          "from": "resolved",
          "to": "inProgress"
        }
      ]
    },
    "TMF656": {
      "specRef": "TMF656 v4.0.0",
      "description": "Service Problem Management, August 2020.",
      "entities": [
        {
          "name": "ServiceProblem",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "affectedNumberOfServices",
            "category",
            "creationDate",
            "description",
            "impactImportanceFactor",
            "lastUpdate",
            "name",
            "originatingSystem",
            "priority",
            "problemEscalation"
          ]
        },
        {
          "name": "ServiceProblemEventRecord",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "eventTime",
            "eventType",
            "recordTime",
            "notification",
            "serviceProblem"
          ]
        },
        {
          "name": "ServiceProblemCreateEventPayload",
          "mandatory": [],
          "optional": [
            "serviceProblem"
          ]
        },
        {
          "name": "ServiceProblemStateChangeEventPayload",
          "mandatory": [],
          "optional": [
            "serviceProblem"
          ]
        }
      ],
      "lifecycle": [
        "acknowledged",
        "rejected",
        "pending",
        "held",
        "inProgress",
        "resolved",
        "cancelled",
        "closed"
      ],
      "terminal": [
        "closed"
      ],
      "transitions": [
        {
          "from": "submitted",
          "to": "acknowledged"
        },
        {
          "from": "acknowledged",
          "to": "held"
        },
        {
          "from": "acknowledged",
          "to": "inProgress"
        },
        {
          "from": "held",
          "to": "inProgress"
        },
        {
          "from": "inProgress",
          "to": "resolved"
        },
        {
          "from": "inProgress",
          "to": "held"
        },
        {
          "from": "resolved",
          "to": "closed"
        },
        {
          "from": "resolved",
          "to": "inProgress"
        }
      ]
    },
    "TMF632": {
      "specRef": "TMF632 v4.0.0",
      "description": "Party Release : 19.",
      "entities": [
        {
          "name": "Party",
          "mandatory": [],
          "optional": [
            "id",
            "href",
            "contactMedium",
            "creditRating",
            "externalReference",
            "partyCharacteristic",
            "relatedParty",
            "taxExemptionCertificate",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "PartyCreditProfile",
          "mandatory": [],
          "optional": [
            "creditAgencyName",
            "creditAgencyType",
            "ratingReference",
            "ratingScore",
            "validFor",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "RelatedParty",
          "mandatory": [
            "id",
            "@referredType"
          ],
          "optional": [
            "href",
            "name",
            "role",
            "@baseType",
            "@schemaLocation",
            "@type"
          ]
        },
        {
          "name": "Individual",
          "mandatory": [
            "id"
          ],
          "optional": [
            "href",
            "aristocraticTitle",
            "birthDate",
            "countryOfBirth",
            "deathDate",
            "familyName",
            "familyNamePrefix",
            "formattedName",
            "fullName",
            "gender",
            "generation",
            "givenName"
          ]
        }
      ],
      "lifecycle": [
        "initialized",
        "validated",
        "deceaded"
      ],
      "terminal": [],
      "transitions": []
    },
    "TMF688": {
      "specRef": "TMF688 v4.0.0",
      "description": "My api short documentation",
      "entities": [
        {
          "name": "EventSubscription",
          "mandatory": [
            "id",
            "callback"
          ],
          "optional": [
            "query"
          ]
        },
        {
          "name": "EventSubscriptionInput",
          "mandatory": [
            "callback"
          ],
          "optional": [
            "query"
          ]
        },
        {
          "name": "TopicCreateEventPayload",
          "mandatory": [],
          "optional": [
            "topic"
          ]
        },
        {
          "name": "TopicChangeEventPayload",
          "mandatory": [],
          "optional": [
            "topic"
          ]
        }
      ],
      "lifecycle": [],
      "terminal": [],
      "transitions": []
    }
  }
};
